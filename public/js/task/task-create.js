document.addEventListener('DOMContentLoaded', function () {
    ...
    const fields = [
        ...
        { el: document.getElementById('typeSelect'),      errorId: 'typeSelect-error',      message: 'Bitte einen Aufgabentyp wählen.',    required: true },
    ...
    if (deadlineAt) {
        ...
        deadlineAt.setAttribute('min', `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`);
    }
    ...
    if (messageEl && charCount) {
        ...
        messageEl.addEventListener('input', () => charCount.textContent = messageEl.value.length);
    }
    ...
    function setInvalid(field) {
        ...
        if (div && !div.dataset.serverError) div.textContent = field.message;
    }
    ...
    form.addEventListener('submit', function (e) {
        const valid = fields.every(field => {
            if (validateField(field)) { setValid(field.el); return true; }
            setInvalid(field); return false;
        });
